...
test_that("runs without errors",{
    expect_silent(use_plumber_microservice(path = path))
})
...
test_that("adds all package dependencies",{
    file_fs <- system.file("configurations", "fs.yml", package = "microservices", mustWork = TRUE)
    packages <- config::get("use_plumber_microservice", file = file_fs)$dependencies
    desc <- desc::description$new(file.path(path, "DESCRIPTION"))

    expected_dependencies <- as.data.frame(config::get("use_plumber_microservice", file = file_fs)$dependencies)
...
test_that("runs without errors",{
    expect_null(add_plumber_service(path = path, name = name))
})
